"""This module defines a Django Rest Framework serializer for the CustomUser
model.
...
By hashing passwords both when creating new users and updating existing ones, it ensures
...
class UserSerializer(serializers.ModelSerializer):
    ...
    Ensuring secure password handling by hashing passwords upon creation
    and update.
    """

    # Using CharField with write_only=True for password handling
    # to ensure the password hash is never sent back to the client.
    ...
    def create(self, validated_data) -> CustomUser:
        """Create and return a new CustomUser instance, given the validated
        data.
    ...
    def update(self, instance: CustomUser, validated_data) -> CustomUser:
        """Update and return an existing CustomUser instance, given the
        validated data.

        If a password is provided, it is hashed before saving the updated user instance.
        Otherwise, updates the instance with the provided data without changing the
        password.

        :param instance: The CustomUser instance to update.
        :param validated_data: Data containing the update data, validated by the
            serializer.
